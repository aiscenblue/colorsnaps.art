...
export const Pin = React.memo(function Pin({ pin, onSelectPin, onRemovePin }: { pin: PinType, onSelectPin: (pin: PinType) => void, onRemovePin?: (pinId: string) => void }) {
    const [imageError, setImageError] = useState(false);
    const [isLoading, setIsLoading] = useState(true); // Add isLoading state

    return (
        <div className="break-inside-avoid mb-4 group">
          <div className="relative rounded-lg bg-white border-2 border-primary transition-all hover:border-4 cursor-pointer" onClick={() => onSelectPin(pin)}>
            {imageError ? (
              <div className="w-full h-48 flex items-center justify-center bg-secondary rounded-t-md">
